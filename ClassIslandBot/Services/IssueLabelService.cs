...
public class IssueLabelService
{
    ...
    private const string Prompt = 
        ...
        - 输出是一个只包含由标签名称组成的 JSON 字符串数组，不要包含其它内容，不要用代码块包裹。
        - 按原样输出标签名，不要做任何修改。
        - 选择最能精确描述输入 Issue 的 1 ~ 2 个标签，标签名应尽可能少，最多不得超过 3 个标签。如果没有适合的标签，请返回空数组。
        
        ## 概念
        
        以下是和 Issue 内容中可能出现的一些概念：
        
        - **档案**：包含课表、时间表和科目信息的集合体
        ...
        | 档案 - 时间表 | |
        | 档案 - 课表 | |
        | 档案 - 课表启用 | 课表启用规则、临时层/课表/课表群、显示次日课表 |
        | 档案 - 课表群 | |
        | 档案编辑器 | 与档案编辑用户界面相关 |
        | 欢迎向导 | |
        | 精确时间服务 | |
